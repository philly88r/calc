...
async function main() {
  ...
  try {
    ...
    console.log(`- Required Post Height: ${requiredHeight} feet`);
    ...
    if (postResult.rows.length === 0) {
      ...
      if (flexibleResult.rows.length > 0) {
        ...
        flexibleResult.rows.forEach(row => {
          console.log(`- ${row.material} ${row.diameter} ${row.thickness} ${row.height}ft: $${row.price} (SKU: ${row.sku})`);
        });
      }
    } else {
      ...
      console.log(`- Description: ${post.material} ${post.diameter} ${post.thickness} ${post.height}ft post`);
